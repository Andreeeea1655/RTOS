...
void rtos_task_create(void (*task_fn)(void), uint32_t priority){
    ...
    stack[size - 2] = ((uint32_t)task_fn) & ~1u;  // PC = functia task-ului
    stack[size - 3] =(uint32_t)rtos_task_exit_trap; // LR pentru thread mode cu PSP (stiva separata)
    stack[size - 4] = 0;                    // R12
    stack[size - 5] = 0;                    // R3
    stack[size - 6] = 0;                    // R2
    stack[size - 7] = 0;                    // R1
